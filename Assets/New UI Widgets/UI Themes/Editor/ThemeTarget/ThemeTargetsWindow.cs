...
namespace UIThemes.Editor
{
	...
	public class ThemeTargetsWindow : EditorWindow
	{
		...
		protected abstract class OptionsSearch<T>
		{
			...
			/// <summary>
			/// Search options in the specified gameobjects.
			/// </summary>
			/// <param name="gameObjects">GameObjects.</param>
			/// <returns>ThemeTargets that match the query.</returns>
			protected virtual IEnumerable<ThemeTargetBase> Search(List<GameObject> gameObjects)
			{
			...
			/// <returns>Assets paths.</returns>
			protected virtual List<string> GetAssetsPaths(string filter, params string[] paths)
			{
				if (paths.Length == 0)
				{
					paths = new[] { "Assets" };
		...
		protected class OpenScenesSearch : OptionsSearch<ThemeTargetBase>
		{
			...
			{
				var gameobjects = UtilitiesEditor.GetHierarchyRootGameObjects();
				var total = (float)gameobjects.Count;
				foreach (var target in Search(gameobjects))
				{
					Result.Add(target);
		...
		protected class AllScenesSearch : OptionsSearch<SceneReference>
		{
			...
			protected override void Search()
			{
				...
				var paths = GetAssetsPaths("t:scene", AssetsPath);
				...
				for (var i = 0; i < paths.Count; i++)
				{
					...
					{
						foreach (var target in Search(UtilitiesEditor.GetHierarchyRootGameObjects()))
						{
		...
		protected class PrefabsSearch : OptionsSearch<PrefabReference>
		{
			...
			protected override void Search()
			{
				var paths = GetAssetsPaths("t:prefab", AssetsPath);
				ProgressShow(0f);
